...
function saveAccounts(accounts) {
  localStorage.setItem(ACCOUNT_ACCOUNTS_KEY, JSON.stringify(accounts));
}
...
function setMessage(text, error = false) {
  ...
  accountMessage.className = `account-message ${error ? "error" : ""}`;
}

function initials(name) {
  return String(name || "ioEz").trim().split(/\s+/).slice(0, 2).map(part => part[0]).join("").toUpperCase() || "IO";
}
...
function renderAccount() {
  ...
  if (account) {
    ...
    if (accountIdentity) accountIdentity.textContent = account.name;
    if (accountEmailLabel) accountEmailLabel.textContent = account.email;
    if (accountAvatar) accountAvatar.textContent = initials(account.name);
    if (accountModeLabel) accountModeLabel.textContent = "Cuenta activa";
    accountPanel?.classList.add("has-account");
  } else {
    ...
    if (accountModeLabel) accountModeLabel.textContent = "Inicia sesión para continuar";
    accountPanel?.classList.remove("has-account");
  }
}
...
function openAccount(mode = "login") {
  ...
  accountSubtitle.textContent = mode === "login" ? "Vuelve a tu espacio de ioEz." : "Crea tu perfil para guardar tu sesión en este navegador.";
  ...
  setTimeout(() => (mode === "login" ? accountEmail : accountName)?.focus(), 80);
}
...
accountForm?.addEventListener("submit", event => {
  ...
  if (!/^\S+@\S+\.\S+$/.test(email)) {
    setMessage("Escribe un correo válido.", true);
    return;
  }
  ...
  if (accountMode === "register") {
    if (accounts.some(account => account.email === email)) {
      setMessage("Ya existe una cuenta con ese correo.", true);
      return;
    }
    ...
    if (!account) {
      setMessage("Correo o contraseña incorrectos.", true);
      return;
    }
