...
function createChat() {
  return { id: crypto.randomUUID(), title: "Nuevo chat", updatedAt: Date.now(), messages: [] };
}
...
function saveChats() {
  ...
  localStorage.setItem(OLD_HISTORY_KEY, JSON.stringify(activeChat?.messages || []));
  renderHistory();
}

function makeTitle(text) {
  const clean = String(text || "Nuevo chat").replace(/\s+/g, " ").trim();
  if (!clean) return "Nuevo chat";
  return clean.length > 42 ? `${clean.slice(0, 42)}…` : clean;
}

function get activeChat() {
...
function renderHistory() {
  ...
  if (!visible.length) {
    historyList.innerHTML = `<div class="history-empty">${query ? "No encontré chats con ese nombre." : "Todavía no hay conversaciones."}</div>`;
    return;
  }
...
function renderText(text) {
  ...
  parts.forEach(part => {
    ...
    if (codeMatch) {
      ...
      wrap.append(head, pre); fragment.appendChild(wrap); return;
    }
...
function addMessageTools(body, text, userText) {
  ...
  const speak = document.createElement("button"); speak.textContent = "🔊 Leer";
  speak.onclick = () => { if (!window.speechSynthesis) return; speechSynthesis.cancel(); const u = new SpeechSynthesisUtterance(text); u.lang = "es-ES"; speechSynthesis.speak(u); };
...
function renderActiveChat() {
  ...
  else chat.messages.forEach(m => addMessage(m.role === "user" ? "user" : "assistant", m.content, { userText: findPreviousUser(chat.messages, m) }));
  renderHistory();
}

function findPreviousUser(list, message) {
  const index = list.indexOf(message);
  if (index < 0) return "";
  for (let i = index - 1; i >= 0; i--) if (list[i].role === "user") return list[i].content;
  return "";
}

async function startOnline() {
  mode = "online"; modeSelect.value = mode; localStorage.setItem(MODE_KEY, mode);
  if (!localStorage.getItem(KEY_NAME)) { settings.classList.remove("hidden"); setStatus("Falta la API key"); addMessage("assistant", "Necesito tu API key de OpenRouter. Pégala en Ajustes. No la subas a GitHub."); return; }
  activateOnline();
}
...
async function loadLocalAI() {
  mode = "local"; modeSelect.value = mode; localStorage.setItem(MODE_KEY, mode);
...
async function askOnline() {
  ...
  return data?.choices?.[0]?.message?.content?.trim() || "No recibí respuesta.";
}
...
async function searchImage(query) {
  const params = new URLSearchParams({ action: "query", generator: "search", gsrsearch: query, gsrnamespace: "6", gsrlimit: "6", prop: "imageinfo", iiprop: "url|extmetadata", iiurlwidth: "800", format: "json", origin: "*" });
  const response = await fetch(`${WIKIMEDIA_ENDPOINT}?${params}`); const data = await response.json(); const pages = Object.values(data?.query?.pages || {});
  return pages.map(page => ({ title: page.title?.replace(/^File:/, "") || "Imagen", url: page.imageinfo?.[0]?.thumburl || page.imageinfo?.[0]?.url, source: "Wikimedia Commons" })).filter(x => x.url);
}

function detectProfileImageRequest(text) {
  const patterns = [/(?:ponte|pon|cámbiate|cambia|usa|ponme)\s+(?:una\s+)?(?:imagen|foto)\s+(?:de|del|de la)\s+(.+)/i, /(?:tu|tus)\s+(?:foto|imagen)\s+(?:de perfil|avatar)\s+(?:sea|como)\s+(.+)/i, /(?:usa|pon|cambia)\s+(?:tu\s+)?(?:foto|imagen)\s+(?:de perfil|avatar)\s+(.+)/i];
  for (const pattern of patterns) { const match = text.match(pattern); if (match?.[1]) return normalizeImageQuery(match[1].replace(/[.!?]+$/, "")); }
  return null;
}
function detectImageSearchRequest(text) {
  const patterns = [/(?:busca|muéstrame|muestrame|enséñame|enseñame|dame)\s+(?:una\s+)?(?:imagen|foto|fotografía)\s+(?:de|del|de la)\s+(.+)/i, /(?:imagen|foto|fotografía)\s+de\s+(.+)/i];
  for (const pattern of patterns) { const match = text.match(pattern); if (match?.[1]) return normalizeImageQuery(match[1].replace(/[.!?]+$/, "")); }
  return null;
}
...
async function handleImageCommand(text) {
  ...
  try {
    ...
    if (profileQuery) {
      ...
      const preview = document.createElement("img"); preview.className = "image-result avatar-preview"; preview.src = selected.url; preview.alt = selected.title; preview.loading = "lazy";
      const caption = document.createElement("small"); caption.className = "image-caption"; caption.textContent = selected.title; bubble.append(preview, caption);
    } else {
      bubble.innerHTML = ""; bubble.appendChild(renderText(`Encontré esto para “${searchQuery}”:`)); const grid = document.createElement("div"); grid.className = "image-grid";
      results.slice(0, 4).forEach(item => { const card = document.createElement("a"); card.className = "image-card"; card.href = item.url; card.target = "_blank"; card.rel = "noopener"; const img = document.createElement("img"); img.src = item.url; img.alt = item.title; img.loading = "lazy"; card.appendChild(img); const title = document.createElement("span"); title.textContent = item.title; card.appendChild(title); grid.appendChild(card); });
      bubble.appendChild(grid);
    }
    const chat = ensureActiveChat(); chat.messages.push({ role: "user", content: text }); chat.messages.push({ role: "assistant", content: `${profileQuery ? "Imagen de perfil actualizada" : "Resultados de imágenes"}: ${searchQuery}` }); chat.updatedAt = Date.now(); if (chat.title === "Nuevo chat") chat.title = makeTitle(text); saveChats(); return true;
...
function restoreAvatar() { document.querySelectorAll(".avatar, .welcome-icon").forEach(setAvatarElement); }
...
async function askAI(text) {
  ...
  const chat = ensureActiveChat();
  chat.messages.push({ role: "user", content: text }); chat.updatedAt = Date.now(); if (chat.title === "Nuevo chat") chat.title = makeTitle(text); saveChats();
  ...
  try {
    ...
    chat.messages.push({ role: "assistant", content: answer }); chat.updatedAt = Date.now(); saveChats();
  } catch (error) { bubble.textContent = `Error: ${error.message}`; chat.messages.pop(); chat.updatedAt = Date.now(); saveChats(); }
...
async function regenerate(userText) {
  ...
  const chat = ensureActiveChat(); const index = chat.messages.map(m => m.role).lastIndexOf("user");
  if (index < 0) return;
  chat.messages = chat.messages.slice(0, index); saveChats(); renderActiveChat(); await askAI(userText);
}

$("composer").addEventListener("submit", async e => { e.preventDefault(); const text = input.value.trim(); if (!text || generating || input.disabled) return; input.value = ""; input.style.height = "auto"; $("welcome")?.remove(); addMessage("user", text); await askAI(text); });
...
function enforceTheme() { applyTheme(localStorage.getItem(THEME_KEY) || "light"); }
