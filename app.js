...
function renderRichText(text) {
  ...
  for (let i = 0; i < parts.length; i++) {
    if (i % 3 === 0) {
      ...
      copy.onclick = async () => {
        try { await navigator.clipboard.writeText(code.trim()); copy.textContent = "Copiado ✓"; setTimeout(() => copy.textContent = "Copiar código", 1200); }
...
function addMessage(role, text, options = {}) {
  ...
  if (role === "assistant") {
    ...
    if (profileData && options.avatarUser) {
      const img = document.createElement("img"); img.src = profileData; img.alt = "Perfil"; avatar.appendChild(img);
    } else avatar.textContent = "Ø";
  ...
  if (role === "user" && profileData) {
    const img = document.createElement("img"); img.className = "user-avatar"; img.src = profileData; img.alt = "Tu perfil"; row.appendChild(img);
  }
...
function renderSavedChat() {
  ...
  history.forEach(m => addMessage(m.role === "user" ? "user" : "assistant", m.content, { avatarUser: false, userText: findPreviousUserText(m) }));
}

function findPreviousUserText(message) {
  const index = history.indexOf(message);
  for (let i = index - 1; i >= 0; i--) if (history[i]?.role === "user") return history[i].content;
  return "";
}
...
function activateOnline() {
  ...
  setStatus("ioEz online // activo"); setReady(true);
  if (!history.length) addMessage("assistant", "Conexión establecida. Soy ioEz. Habla."); else renderSavedChat();
}
...
function activateLocal() {
  setStatus("ioEz local // activo"); setReady(true);
  if (!history.length) addMessage("assistant", "Modo local activo. Sin nube. Sin teatro. Pregunta."); else renderSavedChat();
}
...
async function askOnline() {
  ...
  const data = await response.json().catch(() => ({})); if (!response.ok) throw new Error(data?.error?.message || `HTTP ${response.status}`);
...
async function askAI(text) {
  ...
  try {
    ...
    bubble.innerHTML = ""; bubble.appendChild(renderRichText(answer)); addMessageTools(bubble.parentElement, answer, text);
...
async function regenerate(row, userText) {
  ...
  const index = history.map(m => m.role).lastIndexOf("user"); if (index >= 0) history = history.slice(0, index);
  saveHistory(); row.remove(); await askAI(userText);
}

function openImageGenerator(initial = "") {
  imageComposer.classList.remove("hidden"); imagePrompt.value = initial; imagePrompt.focus();
}

async function generateImage() {
  const prompt = imagePrompt.value.trim(); if (!prompt) return;
  const safePrompt = encodeURIComponent(prompt);
  const url = `${IMAGE_ENDPOINT}${safePrompt}?width=1024&height=1024&nologo=true&safe=true`;
  const userRow = document.createElement("div"); userRow.className = "message user";
  const userBody = document.createElement("div"); userBody.className = "message-body";
  const userBubble = document.createElement("div"); userBubble.className = "bubble"; userBubble.textContent = `🎨 Crear imagen: ${prompt}`; userBody.appendChild(userBubble); userRow.appendChild(userBody);
  ...
  const avatar = document.createElement("div"); avatar.className = "avatar"; avatar.textContent = "Ø";
  const body = document.createElement("div"); body.className = "message-body";
  const bubble = document.createElement("div"); bubble.className = "bubble image-card";
  const img = document.createElement("img"); img.className = "inline-img"; img.alt = prompt; img.loading = "lazy";
  const cap = document.createElement("figcaption"); cap.textContent = "Imagen generada con el prompt indicado.";
  bubble.textContent = "Generando imagen…"; body.appendChild(bubble); row.append(avatar, body); messages.appendChild(row); scrollDown();
  try {
    await new Promise((resolve, reject) => { img.onload = resolve; img.onerror = reject; img.src = url; });
    bubble.innerHTML = ""; bubble.append(img, cap);
    ...
    actions.append(open, copy); body.appendChild(actions);
...
function addUploadedImage(file) {
  ...
  reader.onload = () => {
    ...
    const body = document.createElement("div"); body.className = "message-body";
    const bubble = document.createElement("div"); bubble.className = "bubble image-card";
    const img = document.createElement("img"); img.className = "inline-img"; img.src = reader.result; img.alt = file.name;
    const cap = document.createElement("figcaption"); cap.textContent = file.name;
    bubble.append(img, cap); body.appendChild(bubble); row.appendChild(body);
    ...
    input.disabled = false; send.disabled = false; input.value = `He subido una imagen llamada "${file.name}". ¿Qué puedes decirme de ella?`; input.focus();
  };
...
function setProfile(file) {
  if (!file?.type.startsWith("image/")) return;
  const reader = new FileReader(); reader.onload = () => { profileData = String(reader.result); localStorage.setItem(PROFILE_KEY, profileData); updateProfilePreview(); };
  reader.readAsDataURL(file);
}

function updateProfilePreview() {
  const preview = $("profilePreview"); if (!preview) return;
  if (profileData) { preview.src = profileData; preview.style.display = "block"; } else { preview.removeAttribute("src"); preview.style.display = "none"; }
}
